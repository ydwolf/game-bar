// Minesweeper — 6x6, dynamite is buried only after your first dig.
(() => {
  const ROWS = 6;
  const COLS = 6;
  const MINES = 6;
  ...
  function showBest() {
    const best = Arcade.getBest("minesweeper");
  ...
  function createBoard() {
    ...
    for (let r = 0; r < ROWS; r++) {
      ...
      for (let c = 0; c < COLS; c++) {
        ...
        el.addEventListener("click", () => (flagMode ? toggleFlag(r, c) : reveal(r, c)));
  ...
  // Dynamite is placed after the first dig, avoiding the dug tile and its
  // neighbors, so the first dig always opens up an area of the map.
  ...
  function endGame(won) {
    ...
    board.flat().forEach((cell) => {
      if (cell.mine && !cell.flagged) {
        cell.el.textContent = won ? "🚩" : cell.el.classList.contains("exploded") ? "💥" : "🧨";
    ...
    if (won) {
      ...
      if (!best || seconds < best) {
        Arcade.saveBest("minesweeper", seconds);
        record = true;
        showBest();
      }
    }
    ...
    messageEl.textContent = won
      ? `Claim staked in ${seconds}s!${record ? " New best!" : ""}`
      : "Kaboom! You hit the dynamite.";
    messageEl.className = `mine-message ${won ? "win" : "lose"}`;
  }
  ...
  function addLongPress(el, callback) {
    ...
    el.addEventListener("touchend", (e) => {
      cancel();
      if (longPressed) e.preventDefault(); // don't also fire a dig
    });
  ...
  function setFlagMode(on) {
    ...
    modeBtn.textContent = on ? "🚩 Flag mode" : "⛏️ Dig mode";
  }
  ...
  function newGame() {
    ...
    messageEl.textContent = "Pick any tile to start.";
    messageEl.className = "mine-message";
    setFlagMode(false);
    createBoard();
  }
